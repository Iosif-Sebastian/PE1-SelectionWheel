	...
	public Wheel(ArrayList<String> listOfStrings)  {
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				double distance = Math.sqrt(Math.pow(_mouseDragPosition.getX() - _center.getX(),2) + Math.pow(_mouseDragPosition.getY() - _center.getY(),2));
				if(distance <= _radius)
				{
					spinStop();
				}
		...
		addMouseMotionListener(new MouseAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				...
				double k2 = (mousePos.getY() - _rotationCenter.getY()) / (mousePos.getX() - _rotationCenter.getX());
